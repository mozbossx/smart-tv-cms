...
const showNotification = (data) => {
    ...
    notificationDiv.dataset.annId = data.ann_id;
    ...
    if (userType == 'Admin') {
        notificationDiv.innerHTML = `
            ...
            <p style="color: #6E6E6E"><small>Posted by ${data.ann_author} on ${formattedDate} at ${formattedTime}</small></p>
    ...
    approveButton.onclick = () => showApproveContentModal(data.ann_id);
...
Ws.addEventListener('message', function (event) {
    ...
    if (data.action === 'approve_user') {
        ...
        if (data.success && data.announcement && data.announcement.ann_id) {
            const ann_id = data.announcement.ann_id;
            const pendingContentDiv = document.querySelector(`[data-ann-id="${ann_id}"]`);

            console.log("Looking for pendingContentDiv with ann_id:", ann_id);  // Debugging line
            ...
            if (pendingContentDiv) {
                ...
                console.error("No pendingContentDiv found for ann_id:", ann_id);  // Debugging line
            }
        } else {
            console.error("Invalid data or ann_id missing in response:", data);  // Debugging line
        }
